...
void Parser::putStype ( string s ) {
    ...
    if ( TIDL[ns][c_val].get_type() == LEX_STRUC )
    {
        Number++;
        TIDV.push_back ( { TIDL[s].begin(), TIDL[s].end() } );
    ...
    // номер этого пространства имен будет хранится в поле таблицы TID глобального пространства имен 
}
...
void Executer::execute ( vector<Lex> & poliz )
{
    ...
    while ( index < size ) 
    {
        ...
        case LEX_WRITE:
            ...
            else if ( !TIDV[ns][j.get_value()].get_assign() ) throw " indefinite identifier ";
        ...
        case LEX_READ:
            ...
            {   
                cout << " We dont want to read struct yet" << endl;
            }
        ...
        case LEX_ASSIGN:
            ...
            else if ( !TIDV[ns][i.get_value()].get_assign() ) 
            ...
            else   // присваивание структуры ( меняем нэймспейс )
            {
                ns1 = ns;
                ns = j.get_space();
                TIDV[ns][j.get_value()].put_tidv(ns1);
