// уберем из полиза инициализацию, присовим значения сразу на этапе семиантического анализа 
...
void Parser::putStype ( string s ) {
    ...
    string nsob = TIDL[ns][c_val].get_name().append("Ob");
    TIDL[nsob] = { TIDL[s].begin(), TIDL[s].end() };  
