/* добавлена поддержка пространств имен для корректной работы структур */
...
void Lex::printLex () const 
{
    ...
    else if ( ((int)(t_lex) == 41) ) cout << "Ident: " << TIDL[ns][v_lex].get_name() << endl;
}   
...
void Parser::A ()  // A --> LEX_ID = A | B
{
    ...
    B();                        // после B в st_lex занесется LEX_ID 
    ...
    if ( c_type == LEX_ASSIGN )
    {
        ...
        type_of_lex x;
        from_st(st_lex, x);         // дублируем предпоследний эл-т стека
        dup();
        st_lex.push(x);
...
void Parser::B5 ()   // B5  --> C | LEX_ID | not B5 | -B5 | ( B )
{
    if ( c_type == LEX_ID)
    {
        int val = c_val;     // для того, чтобы описать метку goto и не вылетело исключение 
        gl();
        if ( c_type == LEX_COlON )  // LEX_ID :
        {
            c_val = val;
            dec(LEX_GOTO);
        }
