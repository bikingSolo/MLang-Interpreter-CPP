/*
    Рабочая версия (нету struct + баги)
*/
    ...
    do
    {
        ...
        case H:   // нач состояние
            ...
            else if ( c == '/' )                                      // COM
            {
                gc();
                if ( c == '*') CS = COM;
                else { ungetc(c, fp); buf.push_back(c); CS = DELIM;}
            }
...
class Parser {
    ...
    Scanner      scan;
    void P ();      // Нетерминалы 
    void O();      // читаем одну лексему у Op !
    void C ();
    void Op ();     //  при входе 1-ый токен уже прочитан
    void Ops();
    void B();       //  при входе 1-ый токен уже прочитан
    void B1();
    void B2();
    void B3();
    void B4();
...
void Parser::Op()   // Op -->
{
    ...
    case LEX_ID:  
        ...
        else if ( c_type == LEX_ASSIGN )   // | LEX_ID = B;
        {
            gl();
            B();
